(function () {
  ...
  /* The build moves each animation's poster to data-poster: a poster
     attribute is fetched at page load even under preload="none", so a long
     journal paid for every animation's still before the visitor scrolled
     (USA/Canada: 37 posters, ~1.5 MB). Attach it only as the animation nears
     the viewport, and before the lightbox shows the same video. */
  var revealPoster = function (video) {
    ...
    if ('IntersectionObserver' in window) {
      var animationWarmup = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          ...
          if (entry.isIntersecting) revealPoster(entry.target);
          syncAnimation(entry.target);
        });
      }, { rootMargin: '600px' });
      journalAnimations.forEach(function (video) { animationWarmup.observe(video); });
    } else {
      journalAnimations.forEach(function (video) { video.__nearViewport = true; revealPoster(video); });
