...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=123') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($experience -notmatch 'class="case-study__writeup"' -or $experience -notmatch 'How I built it' -or $experience -notmatch '<strong>reason and select tools in a single inference step</strong>' -or $experience -notmatch 'isolated virtual conversation threads' -or $experience -notmatch 'Specifics are limited by the nature of defence work\.' -or $experience -match 'case-study__expand|case-study__pipeline') { throw "Agentic AI case-study writeup (bolded orchestration mechanisms, confidentiality note, no leftover expand/pipeline markup) is missing or stale." }
