...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=132') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($personal -notmatch 'images/personal/cats-banner\.webp' -or -not (Test-Path -LiteralPath (Join-Path $dist 'images\personal\cats-banner.webp')) -or (Get-Item -LiteralPath (Join-Path $dist 'images\personal\cats-banner.webp')).Length -gt 150KB -or (Test-Path -LiteralPath (Join-Path $dist 'images\personal\cats-instagram-profile.webp'))) { throw "The cats event must publish only its optimized banner asset." }
...
if ($customCss -notmatch '(?s)@media screen and \(max-width: 520px\).*?\.personal-event-card__image--cats img\s*\{[^}]*object-position:\s*center\s+5%.*?\.personal-event-card__image--contain img\s*\{[^}]*padding:\s*0[^}]*object-fit:\s*cover' -or $customCss -match '\[data-category="(?:service|community|build|memory)"\]') { throw "Mobile personal thumbnails must keep both cats in frame and fill the tile, without reviving the unused category accent colours." }
$primaryPagers = @{
