...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=126') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($experience -notmatch 'class="case-study__writeup"' -or $experience -match 'case-study__writeup-label' -or $experience -notmatch 'Spearheaded the design and implementation of a LangGraph-based agentic threat-hunting solution' -or $experience -notmatch 'context engineering' -or $experience -notmatch 'on-premise environment' -or $experience -notmatch 'Specifics are limited by the nature of defence work\.' -or $experience -match 'case-study__expand|case-study__pipeline') { throw "Agentic AI case-study writeup (owner copy without the eyebrow label, confidentiality note, no leftover expand/pipeline markup) is missing or stale." }
...
if ($customCss -notmatch '(?s)\.page-experience #agentic-ai \.case-study__highlights\s*\{[^}]*padding:\s*5\.2rem 0 0 2\.25rem' -or $customCss -notmatch '(?s)\.page-experience #agentic-ai \.case-study__tags\s*\{[^}]*grid-column:\s*1 / -1' -or $customCss -notmatch '(?s)@media screen and \(max-width: 980px\).*?\.page-experience #agentic-ai \.case-study__writeup\s*\{[^}]*margin-top:\s*0\.75rem' -or $experience -notmatch '(?s)aria-label="Agentic AI case study capabilities".*?</ul>\s*<p class="case-study__writeup-note"') { throw "Agentic card layout: timeline starts level with the title band, tags span the card as a base strip, the single-column writeup keeps its breathing room, and the defence note is the card's last line." }
