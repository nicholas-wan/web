...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=128') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($homeHtml -notmatch 'data-scramble-list' -or $homeHtml -notmatch 'data-scramble-source>I build agents that plan threat hunts in hours, not&nbsp;days\.<' -or $homeHtml -notmatch 'data-scramble-row data-scramble-lead="2"' -or $homeHtml -notmatch 'Product direction <span aria-hidden="true">&middot;</span> DSTA' -or $homeHtml -notmatch 'assets/js/scramble-reveal\.js\?v=9' -or $homeHtml -match 'Analysts drown|portfolio-hero__line--|class="txt-rotate"') { throw "Homepage must use the one-sentence scramble hero with the DSTA support line and two unscrambled lead words." }
...
if ($experience -notmatch 'data-scramble-list' -or $experience -notmatch 'data-scramble-source>Planning reduced from days to hours<' -or $experience -notmatch 'data-scramble-row data-scramble-lead="1"' -or $experience -notmatch 'assets/js/scramble-reveal\.js\?v=9') { throw "The experience page must scramble its Threat Hunting outcome (the site's second moment, with the homepage hero), with one unscrambled lead word." }
...
if ($experience -match 'case-study__eyebrow' -or $experience -notmatch 'operationalise validated methods as repeatable analyst workflows within strict data-handling and disclosure constraints' -or $experience -notmatch 'New detection capabilities in daily operational use' -or $experience -notmatch '<li>Behavioural baselining</li>' -or $customCss -notmatch '(?s)\.page-experience #cloud-analytics \.case-study__highlights li::before\s*\{[^}]*background:\s*#11807d' -or $customCss -notmatch '(?s)\.page-experience #cloud-analytics \.case-study__highlights li:last-child::before\s*\{[^}]*background:\s*#e8963f') { throw "Cloud Anomaly Analytics must carry the owner lede, the operational-use outcome, method-level tags, and the light dot-timeline with no eyebrow line (owner decisions, Jul 2026)." }
if ($customCss -notmatch '(?s)\.case-study__highlights\.is-complete \[data-scramble-row\]\s*\{[^}]*color:\s*#ffb869' -or $customCss -notmatch '(?s)@media screen and \(max-width: 980px\).*?\.page-experience #agentic-ai \.case-study__highlights li\s*\{[^}]*min-height:\s*0') { throw "The Outcome must resolve into its dot's amber and the single-column timeline must not keep the desktop rail-stretch row heights." }
