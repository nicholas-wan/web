...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=119') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($experience -notmatch 'class="case-study__expand"' -or $experience -notmatch 'How I led it' -or $experience -notmatch 'Analyst directs' -or $experience -notmatch "off-the-shelf products don't address" -or $experience -notmatch 'Specifics are limited by the nature of defence work\.') { throw "Agentic AI case-study expand section (pipeline diagram, decisions, confidentiality note) is missing." }
