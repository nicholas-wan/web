...
if ($pages.Count -ne 16) { throw "Expected 16 generated pages, found $($pages.Count)." }
$retiredPaths = @(
    'league.html',
    'experience_ndu.html',
...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=142' -or $scan -notmatch 'assets/js/util\.js\?v=1' -or $scan -notmatch 'assets/js/main\.js\?v=18') { throw "$($page.Name) is missing the current shared asset versions." }
...
if ([regex]::Matches($australia, '<div class="masonry mason4">').Count -ne 10 -or $australia -notmatch 'assets/js/gallery\.js\?v=12' -or $australia -notmatch 'assets/js/travel-nav\.js\?v=12') { throw "Every Australia destination must retain its compact, swipeable gallery and inline route controls." }
...
if ([regex]::Matches($guangzhou, 'guangzhou-gallery__brick--tall').Count -ne 5 -or $customCss -notmatch '(?s)\.guangzhou-day--compact \.masonry\s*\{[^}]*grid-auto-rows:\s*1fr' -or $customCss -notmatch '(?s)\.guangzhou-gallery__brick--tall\s*\{[^}]*grid-row:\s*span 2' -or $guangzhou -match 'meal-collage-two\.webp') { throw "Vertical Guangzhou meal collages must span exactly two equal desktop rows and the duplicate Day 1 collage must stay removed." }
...
if ($scrambleRevealJs -notmatch 'prefers-reduced-motion: reduce' -or $scrambleRevealJs -notmatch 'TICK_MS = 45' -or $scrambleRevealJs -notmatch 'TARGET_DURATION_MS = 1200' -or $scrambleRevealJs -notmatch 'VIEW_THRESHOLD = 0\.6' -or $scrambleRevealJs -notmatch 'PAGE_REVEAL_DELAY_MS = 1200' -or $scrambleRevealJs -notmatch 'IntersectionObserver' -or $scrambleRevealJs -notmatch 'if \(hasPlayed\) return;' -or $scrambleRevealJs -notmatch 'list\.classList\.add\("is-complete"\)' -or $scrambleRevealJs -notmatch 'function leadOffset' -or $scrambleRevealJs -notmatch 'data-scramble-lead' -or $scrambleRevealJs -notmatch 'list\.closest\("\.case-study"\)' -or $scrambleRevealJs -notmatch 'list\.querySelector\("\[data-scramble-row\]"\)' -or $scrambleRevealJs -notmatch 'window\.addEventListener\("load", observeAfterPageReveal') { throw "Scramble must be a one-shot, duration-capped reveal with a reduced-motion fallback, unscrambled lead words, and Outcome triggers that wait for the page fade and visible text." }
...
if ($personal -notmatch '(?s)personal-event-card__image--slides-3.*?ndu1-800\.jpg.*?ndu2\.jpg.*?ndu3-800\.jpg.*?<span class="personal-event-card__meta">2016&ndash;2017 &middot; Service</span>.*?<strong>Naval Diving Unit</strong>.*?<span>I completed National Service with the Naval Diving Unit\.</span>') { throw "Naval Diving Unit must be a self-contained three-photo timeline gallery." }
...
if ([regex]::Matches($personal, 'personal-event-card__image--slideshow').Count -ne 5 -or $customCss -notmatch '@keyframes personal-card-slideshow' -or $customCss -notmatch '(?s)\.personal-event-card__body > span:not\([^}]*font-weight:\s*300\s*!important') { throw "Personal hover galleries or lighter descriptions are missing." }
