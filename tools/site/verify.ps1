...
if ($homeHtml -notmatch 'data-scramble-list' -or $homeHtml -notmatch 'data-scramble-source>I build agents that plan threat hunts in hours, not&nbsp;days\.<' -or $homeHtml -notmatch 'data-scramble-row data-scramble-lead="2"' -or $homeHtml -notmatch 'Product direction <span aria-hidden="true">&middot;</span> DSTA' -or $homeHtml -notmatch 'assets/js/scramble-reveal\.js\?v=12' -or $homeHtml -match 'Analysts drown|portfolio-hero__line--|class="txt-rotate"') { throw "Homepage must use the one-sentence scramble hero with the DSTA support line and two unscrambled lead words." }
...
if ($experience -notmatch 'class="case-study__writeup"' -or $experience -match 'case-study__writeup-label|case-study__writeup-note|Specifics are limited' -or $experience -notmatch 'Spearheaded the design and implementation of a LangGraph-based agentic threat-hunting solution' -or $experience -notmatch 'context engineering' -or $experience -notmatch 'on-premise environment' -or $experience -match 'case-study__expand|case-study__pipeline') { throw "Agentic AI case-study writeup (owner copy without the eyebrow label or the retired defence note, no leftover expand/pipeline markup) is missing or stale." }
