...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=139' -or $scan -notmatch 'assets/js/util\.js\?v=1' -or $scan -notmatch 'assets/js/main\.js\?v=18') { throw "$($page.Name) is missing the current shared asset versions." }
...
if ($customCss -notmatch '(?s)@media \(hover: none\).*?animation-duration:\s*20s.*?nth-child\(2\)\s*\{\s*animation-delay:\s*4s;\s*\}.*?nth-child\(3\)\s*\{\s*animation-delay:\s*8s;\s*\}.*?nth-child\(4\)\s*\{\s*animation-delay:\s*12s;\s*\}.*?nth-child\(5\)\s*\{\s*animation-delay:\s*16s;\s*\}') { throw "Touch autoplay must give each personal gallery image a four-second slot." }
