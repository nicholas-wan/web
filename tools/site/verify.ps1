...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=135') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($personal -notmatch 'data-personal-timeline' -or $personal -notmatch 'data-timeline-progress' -or $personal -notmatch 'assets/js/personal-timeline\.js\?v=12' -or -not (Test-Path -LiteralPath (Join-Path $dist 'assets\js\personal-timeline.js'))) { throw "Personal page scroll timeline or its interaction script is missing." }
...
if ($personalTimelineJs -notmatch "classList\.toggle\('is-visible', reducedMotion\.matches \|\| reached\)" -or $personalTimelineJs -notmatch 'window\.innerHeight \* 0\.94' -or $personalTimelineJs -notmatch 'eventBounds\.height \* 0\.12' -or $personalTimelineJs -notmatch 'window\.innerHeight \* 0\.55' -or $personalTimelineJs -notmatch "classList\.toggle\('is-gallery-active', event === activeGallery\)" -or $personalTimelineJs -notmatch "document\.querySelector\('\[data-story-dialog\]'\)" -or $personalTimelineJs -notmatch "document\.documentElement\.classList\.add\('personal-story-is-open'\)" -or $personalTimelineJs -notmatch "event\.key !== 'Escape'" -or $personalTimelineJs -notmatch 'restoreStoryFocus' -or $personalTimelineJs -match 'catsPreview|window\.instgrm|mouseenter') { throw "Timeline reveal timing, focused gallery autoplay, dialog Escape behavior, scroll locking, focus restoration, or retired Instagram preview code has regressed." }
...
if ([regex]::Matches($personal, 'personal-event-card__image--slideshow').Count -ne 3 -or $customCss -notmatch '@keyframes personal-card-slideshow' -or $customCss -notmatch '(?s)\.personal-event-card__body > span:not\([^}]*font-weight:\s*300\s*!important') { throw "Personal hover galleries or lighter descriptions are missing." }
foreach ($gallery in [regex]::Matches($personal, '(?s)personal-event-card__image--slideshow">(.*?)</span>')) { if ([regex]::Matches($gallery.Groups[1].Value, '<img\b').Count -ne 5) { throw "Every personal event gallery must rotate exactly five photos." } }
if ($customCss -notmatch '(?s)\.personal-event-card__image img\s*\{[^}]*object-position:\s*var\(--focus-x[^}]*var\(--focus-y' -or $customCss -notmatch '(?s)@media \(hover: none\).*?\.personal-timeline-event\.is-gallery-active \.personal-event-card__image--slideshow img\s*\{[^}]*animation-name:\s*personal-card-slideshow' -or $customCss -notmatch '@keyframes personal-card-slideshow-first' -or $customCss -match '(?s)\.personal-event-card__image--(?:cats|slideshow) img\s*\{[^}]*object-position:\s*center' -or $customCss -match '(?s)\.personal-event-card:(?:hover|focus-visible) \.personal-event-card__image--slideshow img[^{]*\{[^}]*\banimation:\s') { throw "Per-image focal points, focused touch autoplay, smooth loop crossfades, or delay-preserving animation longhands have regressed." }
...
if ($customCss -notmatch '(?s)\.personal-event-card__body\s*\{[^}]*justify-content:\s*center' -or $customCss -notmatch '(?s)\.personal-event-card__instagram\s*\{[^}]*display:\s*inline-flex[^}]*gap:\s*0\.42rem' -or $customCss -notmatch '(?s)\.personal-story-dialog\s*\{[^}]*margin:\s*auto' -or $customCss -notmatch '(?s)\.orientation-memory img\s*\{[^}]*width:\s*100%[^}]*height:\s*auto' -or $customCss -notmatch '(?s)@media screen and \(max-width: 736px\).*?body\.page-personal #return-to-top\s*\{[^}]*display:\s*none\s*!important.*?\.personal-timeline__present,\s*\.personal-timeline__beginning\s*\{[^}]*left:\s*1\.25rem.*?\.personal-event-card__meta,\s*\.personal-event-card__open\s*\{[^}]*font-size:\s*0\.9rem' -or $customCss -match 'personal-event-card__instagram-preview|cats-instagram-phone-screen|cats-instagram__embed' -or $customCss -match 'body\.page-event \.site-pager(?:__link)?\s*\{') { throw "Personal Instagram link, readable mobile cards, centered dialog, unclipped endpoints, or shared event pager treatment is missing." }
if ($customCss -notmatch '(?s)\.personal-story-dialog\s*\{[^}]*position:\s*fixed[^}]*top:\s*50%[^}]*left:\s*50%[^}]*transform:\s*translate\(-50%, -50%\)') { throw "Personal story dialog must remain explicitly centered in the top layer across browsers." }
