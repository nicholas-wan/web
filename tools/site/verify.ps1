...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=122') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($experience -notmatch 'class="case-study__writeup"' -or $experience -notmatch 'How I led it' -or $experience -notmatch '<strong>testable hunt hypotheses</strong>' -or $experience -notmatch "off-the-shelf products don't address" -or $experience -notmatch 'Specifics are limited by the nature of defence work\.' -or $experience -match 'case-study__expand|case-study__pipeline') { throw "Agentic AI case-study writeup (bolded mechanisms, confidentiality note, no leftover expand/pipeline markup) is missing or stale." }
