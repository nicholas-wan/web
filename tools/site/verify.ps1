...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=134') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($personal -notmatch 'data-personal-timeline' -or $personal -notmatch 'data-timeline-progress' -or $personal -notmatch 'assets/js/personal-timeline\.js\?v=11' -or -not (Test-Path -LiteralPath (Join-Path $dist 'assets\js\personal-timeline.js'))) { throw "Personal page scroll timeline or its interaction script is missing." }
...
if ($personalTimelineJs -notmatch "classList\.toggle\('is-visible', reducedMotion\.matches \|\| reached\)" -or $personalTimelineJs -notmatch 'window\.innerHeight \* 0\.94' -or $personalTimelineJs -notmatch 'eventBounds\.height \* 0\.12' -or $personalTimelineJs -notmatch "document\.querySelector\('\[data-story-dialog\]'\)" -or $personalTimelineJs -notmatch "document\.documentElement\.classList\.add\('personal-story-is-open'\)" -or $personalTimelineJs -notmatch "event\.key !== 'Escape'" -or $personalTimelineJs -notmatch 'restoreStoryFocus' -or $personalTimelineJs -match 'catsPreview|window\.instgrm|mouseenter') { throw "Timeline reveal timing, dialog Escape behavior, scroll locking, focus restoration, or retired Instagram preview code has regressed." }
...
if ($customCss -notmatch '(?s)\.personal-event-card__image img\s*\{[^}]*object-position:\s*var\(--focus-x[^}]*var\(--focus-y' -or $customCss -notmatch '(?s)@media \(hover: none\).*?\.personal-event-card__image--slideshow img\s*\{[^}]*animation-name:\s*personal-card-slideshow' -or $customCss -match '(?s)\.personal-event-card__image--(?:cats|slideshow) img\s*\{[^}]*object-position:\s*center' -or $customCss -match '(?s)\.personal-event-card:(?:hover|focus-visible) \.personal-event-card__image--slideshow img[^{]*\{[^}]*\banimation:\s') { throw "Per-image focal points, touch autoplay, or delay-preserving animation longhands have regressed." }
