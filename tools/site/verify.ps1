...
foreach ($page in $pages) {
    ...
    if ($scan -notmatch 'assets/css/main\.css\?v=2' -or $scan -notmatch 'assets/css/custom\.css\?v=136') { throw "$($page.Name) is missing the current shared stylesheet versions." }
...
if ($customCss -notmatch '(?s)@media screen and \(max-width: 520px\).*?\.personal-event-card__image--contain img\s*\{[^}]*padding:\s*0[^}]*object-fit:\s*cover' -or $customCss -match '\[data-category="(?:service|community|build|memory)"\]') { throw "Mobile personal thumbnails must fill the tile, without reviving the unused category accent colours." }
