...
- Desktop tiles are portrait and mobile tiles are 16/9, so no single crop suits both. Each image carries its own `--focus-x`/`--focus-y`; do not reintroduce a per-class `object-position`. The Brawl Stars artwork is intentionally full-bleed at both breakpoints: desktop focuses on the central character, while the near-16/9 mobile tile retains almost the complete artwork.
