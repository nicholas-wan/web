...
- The hero is `100lvh` so the opening phone screen stays fully black in both URL-bar states. On phones its bottom padding also absorbs the `lvh`-minus-`svh` difference, so the hero content stays centred in the visible window while the URL bar is shown; desktop resolves that to the plain 4rem padding. `initHeroSnap` in `game.js` glides a scroll that settles partway across the gated band the rest of the way (900ms, ease-in-out; a new touch or wheel input cancels it). The band's far edge is the hero edge on phones and, on desktop, the scroll position where the About swipe completes, so a downward glide lands with the photo settled in its slot and the reverse glide is armed anywhere across the whole band. Reduced motion scrolls plainly — do not reintroduce CSS mandatory snap, which cannot be slowed and fights the scroll-driven About swipe.
