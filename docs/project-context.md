...
- The cats, house, and pre-wedding cards each rotate exactly five photos. Desktop hover previews stay quick at one second per photo; touch autoplay uses four seconds per photo so mobile visitors can read each image. Set the animation with longhands — the `animation:` shorthand resets `animation-delay` and collapses the stagger.
